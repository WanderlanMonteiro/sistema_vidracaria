...
- **Frontend**: SPA estática (HTML/CSS/JS puro, sem build) em `public/app/`, servida no mesmo domínio da API, com tela de login. Cobre: catálogo técnico (fabricantes, perfis, fórmulas com checklist de liberação e calculadora), tipologias (nome + desenho técnico — imagem real de catálogo/apostila quando existe, ou desenho feito no editor do sistema) e tabela de folgas de instalação, criação de fórmula nova, clientes, obras, orçamentos (item ligado a fórmula e/ou tipologia — escolhida por cartão visual com o desenho técnico, não só por nome —, vidro, preço fechado ou por m², vão fora de esquadro usando sempre a maior largura × maior altura, acessórios avulsos, relatório de compras, relatório de corte — peça a peça por perfil/comprimento, não em metros —, relatório de têmpera, ficha de fabricação consolidando corte+materiais+têmpera numa folha só pra imprimir, e impressão do orçamento item a item com o desenho técnico, sem mostrar largura×altura — só a metragem), financeiro básico (lançamentos de receita/despesa) e relatório de materiais a comprar (estoque abaixo do mínimo). Estoque/compras/produção/qualidade em si ainda não têm tela própria além do que foi listado — só a API (ver `docs/API.md`).
- **Autenticação**: sessão por cookie (`App\Support\Auth`), toda rota exige login exceto `/` e `/auth/login`. Ver `docs/API.md#autenticação`. Usuários agora têm cadastro pela própria API/tela (`/usuarios`), não só direto no banco — ainda sem permissão por papel.
...
| Catálogo Super5 (ferragens para vidro temperado) | Extraído por completo (54 páginas) — **228 acessórios** com imagem de página vinculada, aplicação registrada em 100% dos itens. **63 já ligados a uma tipologia** (41 Correr/Basculante/Maxim-Ar/Pivotante/Giro da extração original + 22 conferidos individualmente contra as tipologias de vidro temperado da apostila — Box Frontal/de Canto, Guarda-corpo, Vitrine, Porta Pivotante Única, Basculante). Os **165 restantes** seguem sem tipologia porque o texto de aplicação não permite escolher uma variante específica sem inventar informação (ver `docs/FONTES.md`). |
| Apostila Técnica para Vidros de Segurança (curso temperado) | Extraída por completo (134 páginas, lida nas duas versões — .docx sem imagem e depois os 2 PDFs originais com desenho/foto) — **21 tipologias novas** de vidro temperado (Box, Spider, Fachada Glazing, Guarda-corpo, Sacada, Sanfonada, Vitrine, Pivotante/Correr/Max-Ar sem marco), cada uma com o desenho técnico real da apostila; **tabela de folgas de instalação** (24 linhas, p.32). Perfis genéricos e os catálogos de ferragem Santa Marina/Blindex da própria apostila ficaram como pendência documentada (ver `docs/FONTES.md`). |
